...
pub struct CreateApprovalRequestsWorkTimeRequest {
    /// 事業所ID（必須）
    ...
    /// false: 勤務時間を修正する   true: 勤務時間を削除する  勤務時間を削除する場合は以下のパラメータは指定しないでください。 - clock_in_at - clock_out_at - lateness_mins - early_leaving_mins - break_records
    #[serde(rename = "clear_work_time", skip_serializing_if = "Option::is_none")]
    pub clear_work_time: Option<bool>,
    /// 勤務開始時間（条件付き必須）   勤務時間を修正する場合は必須
    #[serde(rename = "clock_in_at", skip_serializing_if = "Option::is_none")]
    pub clock_in_at: Option<String>,
    /// 勤務終了時間（条件付き必須）   勤務時間を修正する場合は必須
...
impl CreateApprovalRequestsWorkTimeRequest {
    pub fn new(company_id: i32, target_date: String, approval_flow_route_id: i32) -> CreateApprovalRequestsWorkTimeRequest {
        CreateApprovalRequestsWorkTimeRequest {
            company_id,
            target_date,
            clear_work_time: None,
            clock_in_at: None,
            clock_out_at: None,
            lateness_mins: None,
